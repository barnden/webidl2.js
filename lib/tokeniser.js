...
export const nonRegexTerminals = [
  "-Infinity",
  "FrozenArray",
  "Infinity",
  "NaN",
...
function tokenise(str) {
  ...
  while (lastCharIndex < str.length) {
    ...
    if (result !== -1) {
      ...
      const lineCount = (currentTrivia.match(/\n/g) ?? []).length;

      if (lineCount > 0) {
        line += lineCount;
        lineCharIndex = lastCharIndex;
        column = 0;
      }
      ...
      column = lastCharIndex - lineCharIndex;
  ...
  function attemptTokenMatch(type, { noFlushTrivia } = {}) {
    ...
    if (result) {
      tokens.push({ type, value: result[0], trivia, line, index, column });
      if (!noFlushTrivia) {
        trivia = "";
      }
      return re.lastIndex;
